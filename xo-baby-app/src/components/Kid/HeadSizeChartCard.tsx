...
import {
  ...
  createHeadCircumferenceRecord,
  CreateMeasurementRecordPayload,
  MeasurementRecord,
...
export default function HeadSizeChartCard({ kidID }: HeadChartCardProps) {
  ...
  const chartData = {
    labels,
    datasets: [{ data: dataset, color: () => "#FF73DC", strokeWidth: 3 }],
    legend: ["Progress"],
  };
  ...
  const handleAddRecord = async () => {
    ...
    try {
      ...
      const created: MeasurementRecord = await createHeadCircumferenceRecord(
        token,
        kidID,
        payload
      );
    ...
    <View style={styles.card}>
      <View style={styles.headerRow}>
        <View style={{ flexDirection: "row", alignItems: "center" }}>
          <View style={[styles.icon1, { backgroundColor: "#FF73DC" }]}>
            <Image
              source={require("../../../assets/development/baby.png")}
              width={10}
              height={10}
            />
          </View>
        ...
        <Pressable
          onPress={() => {
            setSelectedRange((prev) =>
              prev === "3m" ? "6m" : prev === "6m" ? "9m" : "3m"
            );
          }}
        >
          <View style={{ flexDirection: "row", alignItems: "center", gap: 9 }}>
            <Text style={styles.subHeader}>
              {rangeOptions[selectedRange].label}
            </Text>
            <Image
              source={require("../../../assets/development/chevron-down.png")}
              width={16}
              height={16}
            />
          </View>
      ...
      <View style={[styles.chartContainer, { backgroundColor: "#FFE5F9" }]}>
        ...
        <Text style={styles.dateRange}>
          {getMonthYearString(fromDate)} - {getMonthYearString(now)}
        </Text>
        <LineChart
          data={chartData}
        ...
        <Text style={styles.progressNote}>
          Showing{" "}
          <Text style={styles.progressHighlight}>average per month</Text>.
        </Text>
        {loading ? (
