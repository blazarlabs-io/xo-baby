...
import {
  ...
  DoctorDiagnosis as ApiDoctorDiagnosis,
} from "@/api/doctorDiagnosisApi";

type DoctorDiagnosisProp = RouteProp<AppStackParamList, "DoctorDiagnosis">;

export default function DoctorDiagnosisScreen() {
  const navigation =
    useNavigation<
      NativeStackNavigationProp<AppStackParamList, "DoctorDiagnosis">
    >();
  const route = useRoute<DoctorDiagnosisProp>();
  ...
  useEffect(() => {
    const loadDiagnoses = async () => {
      ...
      setLoading(true);
      try {
        const fetched = await getDoctorDiagnoses(token, kidId, 20);
  ...
  const handleAddDiagnosis = async () => {
    if (!newDate || !newTime || !newDoctorName || !newDiagnosis || !token)
      return;
    setLoading(true);
    try {
      const payload = {
      ...
      const created = await createDoctorDiagnosis(token, payload);
    ...
    >
      ...
      >
        <View style={styles.componentHeaderContainer}>
          ...
          />
          <Text style={styles.realTimeText}>Doctor Diagnosis</Text>
        </View>
        ...
        <View style={{ gap: 16, marginTop: 16, width: "100%" }}>
          {loading ? (
            ...
            <Text style={styles.noDataText}>No diagnosis records yet</Text>
          ) : (
            diagnoses.map((item) => (
              <View key={item.id} style={styles.cardWrapper}>
                ...
                <View style={styles.cardContent}>
                  <View style={styles.cardHeader}>
                    ...
                    <Text style={styles.description}>Dr. {item.doctorName}</Text>
                    {item.location && (
