...
export class VaccinationService {
  ...
  private async ensureAccess(kidId: string, userId: string) {
    console.log(`🔍 Vaccination access check - kidId: ${kidId}, userId: ${userId}`);
    ...
    if (!hasAccess) {
      ...
      throw new ForbiddenException("You don't have permission to access this child's vaccination records.");
    }

    return { kid, userRole };
  }

  async create(createDto: CreateVaccinationDto, userId: string) {
  ...
  async findAll(queryDto: GetVaccinationDto, userId: string) {
  ...
  async update(id: string, updateDto: UpdateVaccinationDto, userId: string) {
    const docRef = this.firebaseService.getFirestore().collection('vaccinations').doc(id);
